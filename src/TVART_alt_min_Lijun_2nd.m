...
addParameter(p, 'beta_period', beta_period);
addParameter(p, 'period', period);
...
beta = p.Results.beta;
beta_period = p.Results.beta_period;
...
c = cost(X, Y, A, B, C, N, M, T, r, eta, ...
    beta, beta_period, regularization, proximal);
%cost_vec(1) = c;
...
for iter = 1:max_iter
    ...
    if verbosity >= 2
        [c,rmse] = cost(X, Y, A, B, C, N, M, T, r,...
            eta, beta, beta_period, regularization, proximal);
    ...
    if strcmp(regularization, 'spline')
        C = solve_C_cg(X, Y, A, B, C, N, M, T, r, eta, beta, beta_period, period, proximal);
    else
        C = solve_C_proxg(X, Y, A, B, C, N, M, T, r, eta, beta, beta_period, ...
...
function Cnew = solve_C_proxg(X, Y, A, B, C, N, M, T, r, eta, beta, beta_period, ...
    regularization, proximal)
max_iter = 40;
shrinkage = 1/10;
%step_size = 2e-4 / T;
...
diff_ped = setup_p_smoother(T,period);
    ...
    function z = apply_mat(c)
        ...
        Zt = Zt + beta_period * diff_ped'* diff_ped * Ct;
        z = Zt(:);
    end
...
if beta_period > 0
    ...
    reg4 = 0.5 * beta_period * norm(diff_mat * C, 'fro')^2;
    c = c + reg4;
end
...
function D = setup_p_smoother(T,period)
D = sparse(T-period,T);
D(1:T-period+1:end) = 1;
D((T-period)*period+1: T-period+1:end) = -1;
