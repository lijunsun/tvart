...
addParameter(p, 'beta_peroid', beta_peroid);
addParameter(p, 'peroid', peroid);
...
beta = p.Results.beta;
beta_peroid = p.Results.beta_peroid;
peroid = p.Results.peroid;
...
c = cost(X, Y, A, B, C, N, M, T, r, eta, ...
    beta, beta_peroid, regularization, proximal);
%cost_vec(1) = c;
...
for iter = 1:max_iter
    ...
    if verbosity >= 2
        [c,rmse] = cost(X, Y, A, B, C, N, M, T, r,...
            eta, beta, beta_peroid, regularization, proximal);
    ...
    if strcmp(regularization, 'spline')
        C = solve_C_cg(X, Y, A, B, C, N, M, T, r, eta, beta, beta_peroid, proximal);
    else
        C = solve_C_proxg(X, Y, A, B, C, N, M, T, r, eta, beta, beta_peroid, ...
...
var_aff = 0;
for k = 1:T
    Ypred_k = (A .* (lambda' .* C(k, :))) * (B' * X(:,k));
    var_aff = var_aff + norm(Y(:,k) - Ypred_k, 'fro')^2;
    if center
        Ypred_int = A *  diag(lambda' .* C(k, :))  * B(end, :)' * ...
...
for k = 1:T
    ...
    % rhs = rhs + Y(:,k) * ((X(:,k)' * B) .* C(k,:));
    rhs = rhs + Y(:,k) * (ww(k,:) .* C(k,:));
    % sysmat = sysmat + Dk * B' * X(:,k) * X(:,k)' * B * Dk;
    %w = B' * X(:,k);
    %sysmat = sysmat + Dk * w * w' * Dk;
    sysmat = sysmat + Dk * ww(k,:)' * ww(k,:) * Dk;
end
...
for k = 1:T
    %Dk = diag(C(k, :));
    % rhs = rhs + X(:,k) * Y(:,k)' * A * Dk;
    % rhs = rhs + X(:,k) * ((Y(:,k)' * A) .* C(k,:));
    rhs = rhs + X(:,k) * (ww(k,:) .* C(k,:));
end
...
function Cnew = solve_C_proxg(X, Y, A, B, C, N, M, T, r, eta, beta, beta_peroid, ...
    regularization, proximal)
max_iter = 40;
shrinkage = 1/10;
%step_size = 2e-4 / T;
...
function Cnew = solve_C_cg(X, Y, A, B, C, N, M, T, r, eta, beta, beta_peroid, proximal)
maxit = 24;
tol = 1e-4;
...
% the lag for traffic speed data is 144;
reg4 = 0;

if beta_peroid > 0
    diff_mat = setup_p_smoother(T,144);
    reg4 = 0.5 * beta_peroid * norm(diff_mat * C, 'fro')^2;
    c = c + reg4;
end
disp([reg1,reg2,reg3,reg4]);
...
www = B'* squeeze(X);
www = permute(www,[1,3,2]).*permute(www,[3,1,2]);
R = A' * A;
www = www.*R;
www = www.* permute(C',[1,3,2]);
g2 = squeeze(sum(www,1));
g1 = (B'* X) .* (Y' * A)';
g = -g1' + g2';
...
function D = setup_p_smoother(T,peroid)
D = sparse(T-peroid,T);
D(1:T-peroid+1:end) = 1;
D((T-peroid)*peroid+1: T-peroid+1:end) = -1;
